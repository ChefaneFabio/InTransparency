...
/**
 * POST /api/mediation/messages/[id]/reject
 * Body: { reason }
 */
export async function POST(
  ...
  try {
    ...
    if (!scope.isPlatformAdmin) {
      const gate = await checkPremium(message.thread.institutionId, 'mediation.message.reject')
      if (gate) return gate
    }
    ...
    await audit({
      ...
      action: 'mediation.message.reject',
      entityType: 'MediationMessage',
      entityId: id,
      payload: { threadId: message.threadId, reason: reason.trim() },
