...
/**
 * /for-sme — small/medium business segment funnel.
 *
 * Mirrors the for-enterprise structure (hero + capability cards +
 * process card + sticky CTA) and adds the Talentware-influenced
 * sections used across all company segments: hard-numbers row with
 * source footnotes, X-vs-✓ pain/value comparison, and persona
 * blurbs. Lead message: hiring without a recruiter team — 5 free
 * contacts/mo, then €89/mo only when you outgrow it.
 */
...
const PERSONAS = ['founder', 'manager', 'ops'] as const
const COMPARE_ROWS = ['0', '1', '2', '3', '4'] as const

export default function ForSMEPage() {
  const t = useTranslations('forSME')
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      {/* Hero — text + visual side-by-side, parity with for-enterprise */}
      ...
      <main className="container max-w-5xl mx-auto px-4 pb-16 space-y-10">
        {/* Hard-numbers row — Talentware-style */}
        <section>
        ...
        {/* Pain vs value — Talentware X/✓ comparison */}
        <section>
        ...
        {/* Capabilities — 6 cards with concrete + endpoint */}
        <section>
          <h2 className="text-2xl font-semibold tracking-tight mb-6 max-w-2xl">
        ...
        {/* Personas — Talentware-style 3-up */}
        <section>
          <h2 className="text-2xl font-semibold tracking-tight mb-6 max-w-2xl">
            {t('personas.title')}
          </h2>
      ...
      <StickyCTA href="/auth/register/recruiter" text={t('cta.primary')} show />
      <Footer />
    </div>
  )
}
