...
/**
 * /for-its-students — ITS Academy student page.
 *
 * Slim scaffold (hero + metrics + comparison + CTA) with ITS-specific
 * anchor messaging (87% placement, CTS company visibility, project work
 * as evidence). Amber accent matches the institutional /for-its-institutes
 * family.
 */
...
export default function ForITSStudentsPage() {
  const t = useTranslations('forITSStudents')
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-slate-950">
      ...
      <main>
        <EditorialHero
          ...
          cta={
            <div className="flex flex-col sm:flex-row gap-3">
              <Link
                href="/auth/register/student?type=its"
        ...
        >
          <div className="grid sm:grid-cols-3 gap-x-10 gap-y-8 border-t border-slate-200 dark:border-slate-800 pt-8">
            {METRICS.map(idx => (
              <div key={idx}>
                <div className="text-[40px] font-semibold tracking-tight text-amber-600 dark:text-amber-400 tabular-nums leading-none mb-3">
        ...
        >
          <div className="grid md:grid-cols-2 gap-6">
            ...
            <div className="border border-amber-300 dark:border-amber-800 p-6 bg-amber-50/40 dark:bg-amber-950/10">
              <div className="text-[12px] font-medium uppercase tracking-[0.14em] text-amber-700 dark:text-amber-400 mb-4">
      ...
      <Footer />
      <StickyCTA show={showSticky} text={t('cta.button')} href="/auth/register/student?type=its" />
    </div>
  )
}
