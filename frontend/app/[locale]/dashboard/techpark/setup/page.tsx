...
const TECHPARK_PROFILE_FIELDS: ProfileField[] = [
  { key: 'parkName', label: 'Park name', type: 'text' },
  { key: 'parkType', label: 'Type', type: 'badge' },
  ...
  { key: 'memberCompanyCount', label: 'Member companies', type: 'text' },
  { key: 'foundedYear', label: 'Founded', type: 'text' },
  { key: 'subdivisions', label: 'Subdivisions', type: 'array' },
]

export default function TechParkSetupPage() {
  const t = useTranslations('techParkSetup')
  ...
  const handleSendMessage = useCallback(async (text: string) => {
    ...
    try {
      const res = await fetch('/api/ai/onboard-institution', {
        ...
        body: JSON.stringify({ messages: newHistory, currentData: profile, locale, type: 'techpark' }),
      })
  ...
  const localizedFields = TECHPARK_PROFILE_FIELDS.map(f => ({
    ...f,
  ...
  return (
    <ConversationChat
      ...
      canCreate={!!(profile.parkName && profile.city)}
      messages={messages}
      analyzing={analyzing}
